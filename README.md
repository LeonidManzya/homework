...
Функция load_transactions
 ...
 Загружает транзакции из файла (поддерживает JSON и XLSX).

Функция is_rub_currency

 Определяет, является ли значение рублёвой валютой.

Функция find_currency_value

 Находит значение валюты в транзакции.

Функция filter_rub_transactions

 Фильтрует рублёвые транзакции.
...
Проверяет является ли значение NaN (включая разные типы NaN).
